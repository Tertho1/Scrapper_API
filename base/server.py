...
async def verify_doctor(request: VerifyRequest):
    try:
        ...
        if not doctor_info:
            raise HTTPException(status_code=400, detail="Invalid CAPTCHA or registration number")
        ...
        result = {
            ...
            "name": doctor_info.find("h3", {"class": "mb-4 font-weight-bold text-center"}).text.strip(),
            "registration_number": doctor_info.find(
                "h3", {"class": "badge badge-pill badge-success mt-1 mb-3 font-weight-bold d-block text-center text-white"}
            ).text.strip(),
